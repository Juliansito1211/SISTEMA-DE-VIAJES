...
export default function DetalleViaje() {
  ...
  const [reabriendо, setReabriendo] = useState(false)
  ...
  const cargar = () => {
    ...
    viajesApi.obtener(id)
      .then((v) => { setViaje(v); setVehiculos(v.vehiculos || []) })
  ...
  const editable = noIniciado || enCurso
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="page-header">
        ...
        <h1 className="font-bold text-gray-900 truncate flex-1">{viaje.origen} → {viaje.destino}</h1>
      ...
      <main className="px-4 py-4 max-w-lg mx-auto pb-6 space-y-4">
        ...
        <div className="card space-y-3">
          ...
          {viaje.monto_total != null && (
            <div className="flex justify-between items-center pt-2 border-t border-gray-100">
              ...
              <span className="font-bold text-lg text-gray-900">${parseFloat(viaje.monto_total).toFixed(2)}</span>
            </div>
          )}
        ...
        <div className="card">
          ...
          <div className="space-y-3">
            {vehiculos.map((v) => (
              <div key={v.id} className="flex items-start justify-between gap-2 py-2 border-b border-gray-100 last:border-0">
                <div className="flex-1 min-w-0">
                  ...
                  <p className={`text-sm mt-1 font-medium ${v.monto != null ? 'text-gray-900' : 'text-orange-500'}`}>
                    {v.monto != null ? `$${parseFloat(v.monto).toFixed(2)}` : 'Sin monto'}
                  </p>
                </div>
        ...
        {/* Acciones */}
        <div className="space-y-3">
          {noIniciado && user?.perm_iniciar_viaje && (
          ...
          {finalizado && user?.perm_reabrir_viaje && (
            <button onClick={handleReabrir} disabled={reabriendо} className="btn-secondary">
              {reabriendо ? 'Reabriendo...' : 'Reabrir viaje'}
            </button>
          )}
      ...
      {showModalGrua && (
        <ModalGrua
          viajeId={id}
          onSuccess={(v) => { setViaje(v); setShowModalGrua(false) }}
        />
      )}
      ...
      {showFormVehiculo && (
        <FormVehiculo
          ...
          onSuccess={() => { setShowFormVehiculo(false); setVehiculoEditar(null); cargar() }}
      ...
      {showFinalizar && (
        <ValidacionFinalizar
          ...
          onSuccess={(v) => { setViaje(v); setShowFinalizar(false); cargar() }}
